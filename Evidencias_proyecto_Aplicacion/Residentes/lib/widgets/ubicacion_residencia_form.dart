...
/// Formulario de ubicación (Google Maps + pin), igual al paso 3 del registro.
class UbicacionResidenciaForm extends StatefulWidget {
  const UbicacionResidenciaForm({
    super.key,
    required this.inicial,
    required this.onConfirmar,
    this.onSecundario,
    this.etiquetaPrimaria = 'Continuar',
    this.etiquetaSecundaria = 'Anterior',
    this.titulo = 'Ubicación de la Residencia',
  });

  final UbicacionResidenciaInicial inicial;
  final void Function(UbicacionResidenciaResultado resultado) onConfirmar;
  final VoidCallback? onSecundario;
  final String etiquetaPrimaria;
  final String etiquetaSecundaria;
  final String titulo;

  @override
  State<UbicacionResidenciaForm> createState() => _UbicacionResidenciaFormState();
}

class _UbicacionResidenciaFormState extends State<UbicacionResidenciaForm> {
  ...
  void initState() {
    ...
    if (hayCalleNro || d.direccionYaCargada) {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final botones = Row(
          children: [
            if (widget.onSecundario != null) ...[
              Expanded(
                child: OutlinedButton(
                  onPressed: widget.onSecundario,
                  child: Text(widget.etiquetaSecundaria),
                ),
              ),
              const SizedBox(width: 12),
            ],
            Expanded(
              child: ElevatedButton(
                ...
                child: Text(widget.etiquetaPrimaria),
              ),
            ),
          ],
        );
